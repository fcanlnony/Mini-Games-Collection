...
class ChineseChessUI:
    ...
    def __init__(self, logic: ChineseChessLogic, on_game_over=None):
        self.logic = logic
        self.on_game_over = on_game_over
        self.selected = None
        self.valid_moves = []
        self.mode = GameMode.PVP
        self.ai = ChineseChessAI(difficulty=2)
        self.player_color = Player.RED
        self.ai_thinking = False
        self.animating = False  # 动画进行中
        self.last_move = None  # 记录上一步移动 (from_pos, to_pos)
    ...
    def _on_undo_clicked(self, button: Gtk.Button):
        ...
        if self.logic.move_history:
            record = self.logic.move_history[-1]
            self.last_move = (record['from_pos'], record['to_pos'])
        else:
            self.last_move = None
        self.update_display()
    ...
    def _set_piece_style(self, cell: Gtk.Button, piece):
        if piece:
            color = "#c00000" if piece.color == Player.RED else "#000000"
            cell.set_label(piece.char)
            css = f"button {{ color: {color}; }}"
            provider = Gtk.CssProvider()
            provider.load_from_string(css)
            cell.get_style_context().add_provider(
    ...
    def _animate_move(self, from_row: int, from_col: int, to_row: int, to_col: int,
                      piece_char: str, piece_color: Player, callback=None):
        """执行移动动画"""
        self.animating = True
        ...
        # 设置动画棋子样式
        text_color = "#c00000" if piece_color == Player.RED else "#000000"
        css = f"""
            label {{
                font-size: 24px;
                font-weight: bold;
    ...
    def _on_cell_clicked(self, button: Gtk.Button, row: int, col: int):
        ...
        if (row, col) in self.valid_moves:
            ...
            piece_char = piece_at_from.char
            piece_color = piece_at_from.color
            ...
            self._animate_move(from_row, from_col, row, col, piece_char, piece_color, on_animation_done)
            return

        if piece and piece.color == self.logic.current_player:
    ...
    def _apply_ai_move(self, move):
        ...
        if move:
            ...
            self._animate_move(from_row, from_col, to_row, to_col, piece_char, piece_color, on_animation_done)
        else:
            self.update_display()
            if self.logic.is_game_over():
                self._show_game_over()
    ...
    def update_display(self):
        ...
        if self.logic.is_game_over():
            ...
            label = (_("xiangqi_red_in_check") if self.logic.current_player == Player.RED
                     else _("xiangqi_black_in_check"))
            self.status_label.set_label(label)
        else:
            label = (_("xiangqi_red_turn") if self.logic.current_player == Player.RED
                     else _("xiangqi_black_turn"))
            self.status_label.set_label(label)
        ...
        self.captured_red_label.set_label(
            ''.join(p.char for p in self.logic.captured_red)
        )
        self.captured_black_label.set_label(
            ''.join(p.char for p in self.logic.captured_black)
        )

    def _show_game_over(self):
        winner = self.logic.get_winner()
        if winner:
            heading = _("xiangqi_checkmate")
            body = _("xiangqi_red_wins") if winner == Player.RED else _("xiangqi_black_wins")
        else:
            heading = _("game_over")
            body = _("xiangqi_stalemate")
